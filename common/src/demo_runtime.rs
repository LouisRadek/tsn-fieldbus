...
mod tests {
    ...
    fn test_netmask_to_prefix() {
        assert_eq!(netmask_to_prefix_length([255, 255, 255, 0]).unwrap(), 24);
        assert_eq!(netmask_to_prefix_length([255, 255, 0, 0]).unwrap(), 16);
    }

    #[test]
    fn test_netmask_to_prefix_rejects_non_contiguous() {
        assert!(netmask_to_prefix_length([255, 0, 255, 0]).is_err());
    }

    #[test]
    fn test_demo_interface_names_fit_linux_limit() {
        for interface_name in [
            ...
            assert!(
                ...
                "Interface name exceeds Linux limit: {interface_name}"
            );
        }
    }
}
